...
CHAIN IF ~ Global("g_mmMQ1","GLOBAL",1) Global("g_mmTalk","GLOBAL",0)~ THEN d_mmTT firstTalk
	~Ah! Finally! It’s about time you showed up! I apologize for the behavior of the others; I told them to wait until you arrived to eat, but, well, you know how the lower classes can be.~
DO ~IncrementGlobal("g_mmMQ1","GLOBAL",1)~
...
IF ~~ THEN BEGIN enjoy1
	SAY ~Enjoy.~ IF ~~ THEN DO ~ SetGlobal("g_mmTalk","GLOBAL",1)  ~ EXIT
END
...
IF ~~ THEN BEGIN enjoy3
	SAY ~Just let me know when.~ IF ~~ THEN EXIT
END
...
IF ~Global("g_mmMQ1","GLOBAL",1)~ THEN BEGIN death1
	SAY ~Now, where are the drinks? Oh dear. I had told everyone to hold off on the drinks at least until you had settled in! Pelial! Put that down or so help me–wait, is that… Shipmaster Adrik?~ IF ~~ THEN
DO ~   	SetGlobal("g_mmMQ1","GLOBAL",2)
...
IF ~~ THEN DO ~
	SetGlobal("g_mmEng","GLOBAL",1)~ EXIT 
END
...
IF ~~ THEN BEGIN nottasty
	...
	= ~Not that I’m accusing the dead of anything! That would be uncouth. Speak to everyone. I'm sure there's got to be some sort of clue as to who the killer is!~ IF ~~ THEN DO ~
   SetGlobal("g_mmMQ1","GLOBAL",5)~ EXIT
END
...
IF ~~ THEN DO ~
	SetGlobal("g_mmMQ1","GLOBAL",7)  ActionOverride("g_mmTr",EscapeArea()) ActionOverride("g_mmTr",EscapeArea())~ EXIT 
END
